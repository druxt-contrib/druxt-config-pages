...
Thanks for helping. This module follows the Druxt repository standard, and
`examples/module-template` is the worked example of it.
...
scripts, run `yarn hooks:install` by hand, or the hooks stay on disk doing
nothing.
...
```bash
yarn lint     # every check below except prose, which needs Vale installed
yarn test         # jest, with the coverage floor enforced
```
...
`yarn lint:private` checks this and runs in the pipeline. When you need to
